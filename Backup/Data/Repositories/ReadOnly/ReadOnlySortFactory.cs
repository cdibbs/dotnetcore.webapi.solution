...
{
    public class ReadOnlySortFactory<T> : ISortFactory<T> where T: ViewEntity
    {
        public SortSpecification[] Sorts { get; set; }
        public Dictionary<string, Expression<Func<T, object>>> Selectors { get; set; }
        public ReadOnlySortFactory(SortSpecification[] sorts, Dictionary<string, Expression<Func<T, object>>> availableSelectors)
        {
            this.Sorts = sorts;
